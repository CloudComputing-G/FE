...
import {
  ...
  BarChart2, Upload, Users, Bot, ChevronDown, ChevronUp, MessageCircle,
} from "lucide-react";
...
function ChatSelectContent() {
  ...
  function handleSelect(item: SubmittedAssignment) {
    ...
    router.replace(`/student/chat?submissionId=${item.submissionId}`);
  }
    ...
    <div className="flex flex-col w-full max-w-md h-dvh bg-[#F9FAFB] mx-auto">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-6 pb-6 flex flex-col gap-4">
        ...
        {loading ? (
          ...
          <p className="text-[14px] text-[#6B7280] text-center py-16">문항 정보를 불러올 수 없습니다.</p>
        ) : (
          <div className="flex flex-col gap-2">
            {questions.map((q, idx) => {
              ...
              return (
                <Link
                  key={q.questionId}
                  href={`/student/chat/${q.questionId}?submissionId=${selectedSubmissionId}&from=chat-select`}
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                      <span className={cn("text-[12px] font-medium leading-[18px] px-3 py-1 rounded-full", cfg.badge, cfg.text)}>
                        {cfg.label}
                      </span>
